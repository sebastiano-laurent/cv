  ...
  en: {
    ...
    experiences: [
      {
        ...
        bullets: [
          ...
          "Provided technical mentoring and supported code review, testing, and release quality gates."
        ]
      },
      {
        role: "Engitel S.p.A. | Software Developer",
    ...
    project: {
      ...
      bullets: [
        ...
        "Strong focus on accessibility and usability improvements.",
    ...
    devopsSkills: [
      "Git",
      "Azure DevOps (previous CI/CD infrastructure experience)",
      "Google Cloud Console (current API release activities)",
      "Code Review",
      "Testing",
      "Release Support"
    ],
...
const elements = {
  ...
  project: document.getElementById("cv-project"),
...
const themeButtons = Array.from(document.querySelectorAll(".footer-btn[data-theme]"));
...
function initImpactCounters() {
  ...
  if ("IntersectionObserver" in window) {
    ...
    impactObserver.observe(elements.impact);
  } else {
    run();
  }
}
...
function renderCv(lang) {
  ...
  elements.labelProject.textContent = data.projectLabel;
...
langButtons.forEach((button) => {
  button.addEventListener("click", () => {
    renderCv(button.dataset.lang === "en" ? "en" : "it");
  });
});
...
if (profileFlip) {
  window.setTimeout(() => {
    profileFlip.classList.add("is-flipped");
  }, 4200);
